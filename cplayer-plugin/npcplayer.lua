...
local function log_function(filename)
	local f
	return function(...)
		f = f or (filename and io.open(filename, 'w'))
...
--pass nothing to log_function if you don't want logging
...
local create_instance --forward decl. to the hi-level plugin API below
...
local function new_instance(instance, args)
	local id = gen_instance_id()
	local inst = create_instance(args, browser)
	instances[id] = inst
...
local function instance_id(instance)
	return tonumber(ffi.cast('ptrdiff_t', instance.pdata))
end
...
local function free_instance(instance)
	...
	instances[instance_id(instance)] = nil
	instance.pdata = nil
end
...
function npp.NPP_New(mime_type, instance, mode, argc, argn, argv, saved)
	...
	inst = new_instance(instance, args)
end
...
function npp.NPP_SetWindow(instance, window)
	...
	find_instance(instance):set_window(window_type, window.window, window.x, window.y, window.width, window.height)
end

--hi-level plugin API: instance objects

local inst = {}
local inst_meta = {__index = inst}

function create_instance(args, browser) --not global but fwd. decl.
	log('create_instance(...)')
	return setmetatable({args = args, browser = browser}, inst_meta)
end

--thead function. communicates through the linda arg. keep it free of upvalues.
local function thread_function(linda, thread_id)
	local ffi = require'ffi'
	local log = log_function(string.format('x:/work/lua-files/cplayer-plugin/log-%d.txt', thread_id))
	log('GUI thread started')

	local player
	while true do
		if linda:get('free') then
			log('got free')
			return
		end
		local parent = linda:get('play')
		if parent then
			log('got play %d', parent)
			parent = ffi.cast('void*', parent)
			player = require'path_line_demo'
			player.main = player:window{on_render = player.on_render, parent = parent}
...
jit.off(thread_function, true)

local thread_id = 0 --autoincremented thread id for logging

function inst:get_linda()
	if not self.thread then
		package.path = 'x:/work/lua-files/?.lua'
		package.cpath = 'x:/work/lua-files/?.dll;x:/work/lua-files/bin/?.dll'
		local lanes = require'lanes'.configure()
		local create_thread = lanes.gen('*', thread_function)
		self.linda = lanes.linda()
		thread_id = thread_id + 1
		self.thread = create_thread(self.linda, thread_id)
		log('   %-26s %s', '>thread', tostring(self.thread))
	end
	return self.linda
end
...
function inst:free()
	...
	self:get_linda():send('free', true)
	local _ = self.thread[1]
	--self.thread:join(0.5)
end

function inst:set_window(wtype, hwnd, x, y, w, h)
	hwnd = tonumber(ffi.cast('ptrdiff_t', hwnd)) --make it a number so we can pass it to linda
	hwnd = hwnd ~= 0 and hwnd or nil
	log('inst:set_window(%s, %d, %d, %d, %d, %d)', wtype, hwnd, x, y, w, h)
	assert(wtype == 'window') --we only support native windows
	self:get_linda():send('play', hwnd)
end
