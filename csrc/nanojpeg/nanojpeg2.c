...
NJ_INLINE void njSkipBits(nj_context_t* nj, int bits) {
    if (nj->bufbits < bits)
        (void) njShowBits(nj, bits);
    nj->bufbits -= bits;
}

NJ_INLINE int njGetBits(nj_context_t* nj, int bits) {
    int res = njShowBits(nj, bits);
    njSkipBits(nj, bits);
    return res;
}

NJ_INLINE void njByteAlign(nj_context_t* nj) {
    nj->bufbits &= 0xF8;
}

static void njSkip(nj_context_t* nj, int count) {
    nj->pos += count;
    nj->size -= count;
    nj->length -= count;
...
static void njDecodeLength(nj_context_t* nj) {
    if (nj->size < 2) njThrow(nj, NJ_SYNTAX_ERROR);
    nj->length = njDecode16(nj->pos);
    if (nj->length > nj->size) njThrow(nj, NJ_SYNTAX_ERROR);
    njSkip(nj, 2);
}

NJ_INLINE void njSkipMarker(nj_context_t* nj) {
    njDecodeLength(nj);
    njSkip(nj, nj->length);
}

NJ_INLINE void njDecodeSOF(nj_context_t* nj) {
    int i, ssxmax = 0, ssymax = 0;
    nj_component_t* c;
    njDecodeLength(nj);
    if (nj->length < 9) njThrow(nj, NJ_SYNTAX_ERROR);
    if (nj->pos[0] != 8) njThrow(nj, NJ_UNSUPPORTED);
    ...
    switch (nj->ncomp) {
        ...
        case 3:
            break;
        default:
            njThrow(nj, NJ_UNSUPPORTED);
    }
    if (nj->length < (nj->ncomp * 3)) njThrow(nj, NJ_SYNTAX_ERROR);
    for (i = 0, c = nj->comp;  i < nj->ncomp;  ++i, ++c) {
        c->cid = nj->pos[0];
        if (!(c->ssx = nj->pos[1] >> 4)) njThrow(nj, NJ_SYNTAX_ERROR);
        if (c->ssx & (c->ssx - 1)) njThrow(nj, NJ_UNSUPPORTED);  // non-power of two
        if (!(c->ssy = nj->pos[1] & 15)) njThrow(nj, NJ_SYNTAX_ERROR);
        if (c->ssy & (c->ssy - 1)) njThrow(nj, NJ_UNSUPPORTED);  // non-power of two
        if ((c->qtsel = nj->pos[2]) & 0xFC) njThrow(nj, NJ_SYNTAX_ERROR);
        njSkip(nj, 3);
        nj->qtused |= 1 << c->qtsel;
    ...
    for (i = 0, c = nj->comp;  i < nj->ncomp;  ++i, ++c) {
        ...
        if (((c->width < 3) && (c->ssx != ssxmax)) || ((c->height < 3) && (c->ssy != ssymax))) njThrow(nj, NJ_UNSUPPORTED);
        if (!(c->pixels = njAllocMem(c->stride * (nj->mbheight * nj->mbsizey * c->ssy / ssymax)))) njThrow(nj, NJ_OUT_OF_MEM);
    }
    if (nj->ncomp == 3) {
...
NJ_INLINE void njDecodeDHT(nj_context_t* nj) {
    ...
    while (nj->length >= 17) {
        i = nj->pos[0];
        if (i & 0xEC) njThrow(nj, NJ_SYNTAX_ERROR);
        if (i & 0x02) njThrow(nj, NJ_UNSUPPORTED);
        ...
        for (codelen = 1;  codelen <= 16;  ++codelen) {
            ...
            if (!currcnt) continue;
            if (nj->length < currcnt) njThrow(nj, NJ_SYNTAX_ERROR);
            remain -= currcnt << (16 - codelen);
            if (remain < 0) njThrow(nj, NJ_SYNTAX_ERROR);
            for (i = 0;  i < currcnt;  ++i) {
    ...
    if (nj->length) njThrow(nj, NJ_SYNTAX_ERROR);
}

NJ_INLINE void njDecodeDQT(nj_context_t* nj) {
    int i;
    unsigned char *t;
    njDecodeLength(nj);
    while (nj->length >= 65) {
        i = nj->pos[0];
        if (i & 0xFC) njThrow(nj, NJ_SYNTAX_ERROR);
        nj->qtavail |= 1 << i;
        t = &nj->qtab[i][0];
        for (i = 0;  i < 64;  ++i)
            t[i] = nj->pos[i + 1];
...
NJ_INLINE void njDecodeDRI(nj_context_t* nj) {
    njDecodeLength(nj);
    if (nj->length < 2) njThrow(nj, NJ_SYNTAX_ERROR);
    nj->rstinterval = njDecode16(nj->pos);
    njSkip(nj, nj->length);
}

static int njGetVLC(nj_context_t* nj, nj_vlc_code_t* vlc, unsigned char* code) {
...
NJ_INLINE void njDecodeBlock(nj_context_t* nj, nj_component_t* c, unsigned char* out) {
    unsigned char code = 0;
    int value, coef = 0;
    njFillMem(nj->block, 0, sizeof(nj->block));
    c->dcpred += njGetVLC(nj, &nj->vlctab[c->dctabsel][0], NULL);
    nj->block[0] = (c->dcpred) * nj->qtab[c->qtsel][0];
    do {
        value = njGetVLC(nj, &nj->vlctab[c->actabsel][0], &code);
        if (!code) break;  // EOB
        if (!(code & 0x0F) && (code != 0xF0)) njThrow(nj, NJ_SYNTAX_ERROR);
        coef += (code >> 4) + 1;
        if (coef > 63) njThrow(nj, NJ_SYNTAX_ERROR);
...
NJ_INLINE void njDecodeScan(nj_context_t* nj) {
    int i, mbx, mby, sbx, sby;
    int rstcount = nj->rstinterval, nextrst = 0;
    nj_component_t* c;
    njDecodeLength(nj);
    if (nj->length < (4 + 2 * nj->ncomp)) njThrow(nj, NJ_SYNTAX_ERROR);
    if (nj->pos[0] != nj->ncomp) njThrow(nj, NJ_UNSUPPORTED);
    njSkip(nj, 1);
    for (i = 0, c = nj->comp;  i < nj->ncomp;  ++i, ++c) {
        if (nj->pos[0] != c->cid) njThrow(nj, NJ_SYNTAX_ERROR);
        if (nj->pos[1] & 0xEE) njThrow(nj, NJ_SYNTAX_ERROR);
        c->dctabsel = nj->pos[1] >> 4;
        c->actabsel = (nj->pos[1] & 1) | 2;
        njSkip(nj, 2);
    }
    if (nj->pos[0] || (nj->pos[1] != 63) || nj->pos[2]) njThrow(nj, NJ_UNSUPPORTED);
    njSkip(nj, nj->length);
    for (mbx = mby = 0;;) {
        for (i = 0, c = nj->comp;  i < nj->ncomp;  ++i, ++c)
        ...
        if (nj->rstinterval && !(--rstcount)) {
            njByteAlign(nj);
            i = njGetBits(nj, 16);
            if (((i & 0xFFF8) != 0xFFD0) || ((i & 7) != nextrst)) njThrow(nj, NJ_SYNTAX_ERROR);
            nextrst = (nextrst + 1) & 7;
...
NJ_INLINE void njUpsampleH(nj_context_t* nj, nj_component_t* c) {
    const int xmax = c->width - 3;
    unsigned char *out, *lin, *lout;
    int x, y;
    out = njAllocMem((c->width * c->height) << 1);
    if (!out) njThrow(nj, NJ_OUT_OF_MEM);
    lin = c->pixels;
    lout = out;
    for (y = c->height;  y;  --y) {
...
NJ_INLINE void njUpsampleV(nj_context_t* nj, nj_component_t* c) {
...
NJ_INLINE void njUpsample(nj_context_t* nj, nj_component_t* c) {
...
NJ_INLINE void njConvert(nj_context_t* nj) {
    int i;
    nj_component_t* c;
    for (i = 0, c = nj->comp;  i < nj->ncomp;  ++i, ++c) {
        #if NJ_CHROMA_FILTER
            while ((c->width < nj->width) || (c->height < nj->height)) {
                if (c->width < nj->width) njUpsampleH(nj, c);
                njCheckError(nj);
                if (c->height < nj->height) njUpsampleV(nj, c);
                njCheckError(nj);
            }
        #else
            if ((c->width < nj->width) || (c->height < nj->height))
                njUpsample(c);
        #endif
        if ((c->width < nj->width) || (c->height < nj->height)) njThrow(nj, NJ_INTERNAL_ERR);
    }
    if (nj->ncomp == 3) {
        // convert to RGB
        int x, yy;
...
nj_context_t* njInit() {
    nj_context_t* nj;
    nj = njAllocMem(sizeof(nj_context_t));
    if (!nj) return 0;
...
int njGetWidth(nj_context_t* nj)            { return nj->width; }
int njGetHeight(nj_context_t* nj)           { return nj->height; }
int njIsColor(nj_context_t* nj)             { return (nj->ncomp != 1); }
unsigned char* njGetImage(nj_context_t* nj) { return (nj->ncomp == 1) ? nj->comp[0].pixels : nj->rgb; }
int njGetImageSize(nj_context_t* nj)        { return nj->width * nj->height * nj->ncomp; }
