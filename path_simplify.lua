...
local shape_writers = {
	...
	superformula = shape_writer(shapes.superformula, 10),
}
...
local function path_simplify(write, path) --this is drawing code so avoid making garbage in here
	...
	for i,s in path_commands(path) do
		...
		elseif s == 'arc' or s == 'rel_arc' then
			...
			start_angle, sweep_angle = radians(start_angle), radians(sweep_angle)
			local x1, y1 = arc_endpoints(cx, cy, r, start_angle, sweep_angle)
			write(cpx and 'line' or 'move', x1, y1)
			arc_to_bezier3(write, cx, cy, r, start_angle, sweep_angle)
		elseif s == 'arc_3p' or s == 'rel_arc_3p' then
			...
			arc_3p_to_bezier3(write, cpx, cpy, x2, y2, x3, y3)
		elseif s == 'svgarc' or s == 'rel_svgarc' then
			...
			svgarc_to_bezier3(write, cpx, cpy, rx, ry, radians(angle), large_arc_flag, sweep_flag, x2, y2)
		elseif s == 'text' then
