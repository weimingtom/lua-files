...
- bugs:
	- fix angle degradation
	- fix choosing between positive and negative arc
	- make round-corner control points work
...
local function editor(path)
	...
	local function cpline(p1x, p1y, p2x, p2y)
		...
		local cp1x = var(cp_path, #cp_path-1)
		local cp1y = var(cp_path, #cp_path-0)
		expr(cp1x, copy, p1x)
		expr(cp1y, copy, p1y)
		glue.append(cp_path, 'line', val[p2x], val[p2y])
		local cp2x = var(cp_path, #cp_path-1)
		local cp2y = var(cp_path, #cp_path-0)
		expr(cp2x, copy, p2x)
		expr(cp2y, copy, p2y)
	...
	for i,s in path_state.commands(path) do
		...
		elseif s == 'arc' or s == 'rel_arc' then
			...
			local p2x, p2y = point(getp2x(), getp2y())
			local p3x, p3y = point(getp3x(), getp3y())
		...
		elseif s == 'elliptical_arc' or s == 'rel_elliptical_arc' then
			...
			local prxx, prxy = point(getprxx(), val[pcy])
			local pryx, pryy = point(val[pcx], getpryy())
			...
			link(p1x, prxx, function(t) return t[pcx] + getrx() end)
			link(p1y, prxx, function(t) return t[pcx] + getrx() end)
			link(p1x, prxy, copy, pcy)
			link(p1y, prxy, copy, pcy)

			link(p1x, pryx, copy, pcx)
			link(p1y, pryx, copy, pcx)
			link(p1x, pryy, function(t) return t[pcy] + getry() end)
			link(p1y, pryy, function(t) return t[pcy] + getry() end)

			link(p2x, prxx, function(t) return t[pcx] + getrx() end)
			link(p2y, prxx, function(t) return t[pcx] + getrx() end)
			link(p2x, prxy, copy, pcy)
			link(p2y, prxy, copy, pcy)

			link(p2x, pryx, copy, pcx)
			link(p2y, pryx, copy, pcx)
			link(p2x, pryy, function(t) return t[pcy] + getry() end)
			link(p2y, pryy, function(t) return t[pcy] + getry() end)
		...
		elseif s == 'ellipse' then
			...
			local pcx, pcy = point(x, y)
			local prxx, prxy = point(x + rx, y)
			local pryx, pryy = point(x, y + ry)
			constrain(pryx, copy, pcx)
			constrain(prxy, copy, pcy)
...
local path = {
	...
	'move', 1000, 500,
	'rel_elliptical_arc', -500, -200, 0, 0, 1, 100, 100,
}
...
function player:on_render(cr)
	...
	for i=#e.points-1,1,-2 do
		...
		if self.dragging_handle == i then
			e.update(i+0, self.mouse_x)
			e.update(i+1, self.mouse_y)
		end
	end
